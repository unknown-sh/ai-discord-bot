...
async def api_get_config(request: Request, key: str):
    ...
    if not user_id:
        raise HTTPException(status_code=400, detail="Missing user ID for access control")
    ...
    await log_action(user_id, "get_config", {"key": key, "timestamp": datetime.utcnow().isoformat()})
...
async def api_set_config(request: Request, payload: dict):
    ...
    await log_action(user_id, "set_config", {"key": key, "value": value, "timestamp": datetime.utcnow().isoformat()})
...
async def api_show_config(request: Request, key: str):
    ...
    value = await get_config(key)
    await log_action(user_id, "show_config", {"key": key, "timestamp": datetime.utcnow().isoformat()})
    return {"key": key, "value": value}
...
async def api_get_all_config(request: Request):
    ...
    await log_action(user_id, "get_all_config", {"timestamp": datetime.utcnow().isoformat()})
    return result
